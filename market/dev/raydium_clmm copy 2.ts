 ...
 */
async function fetchClmmPoolData(poolId: string): Promise<{
  // config: ClmmConfigInfoSerializable;
  pool: ClmmPoolInfoSerializable;
}> {
  try {
    // Fetch pool account data
    const poolData = await fetchAccountData(connection, poolId, RAYDIUM_CLMM_PROGRAM_ID);
    ...
    throw new Error(`Failed to fetch CLMM pool data: ${(error as Error).message}`);
  }
}
...
/**
 * Simple CLI to accept CLMM pool public key as an argument.
 */
async function main() {
  const args = process.argv.slice(2);
  if (args.length !== 1) {
    console.error('Usage: ts-node raydium_clmm.ts <CLMM_POOL_PUBLIC_KEY>');
    process.exit(1);
  }
  ...
  try {
    ...
    // console.log('--- CLMM Config Info ---');
    // console.log(JSON.stringify(clmmData.config, null, 2));
    // console.log('--- CLMM Pool Info ---');
    // console.log(JSON.stringify(clmmData.pool, null, 2));
  } catch (error) {
    console.error(error);
  }
}
